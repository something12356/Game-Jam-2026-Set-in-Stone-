...
class ScreenInfo:
    def from_sc_size(self, sc_size: Vec2):
        ...
        self.player_left_area = self.base_player_area.scale_by(0.2, 1).move_to(
            topleft=(0, 0))
        self.player_ores_area = self.player_left_area.scale_by(1, 0.6).move_to(
            topleft=self.player_left_area.topleft)
        self.player_buttons_area = self.player_left_area.scale_by(1, 0.4).move_to(
            topleft=self.player_ores_area.bottomleft)
        self.player_right_area = self.base_player_area.scale_by(0.8, 1).move_to(
...
class Player:
    ...
    def render_side_buttons(self, dest: pygame.Surface):
        ...
        self.buttons += [(txr, lambda: self.factory.add_ore("FireOpal", 1))]
